...
LDAP_CONFIGURATOR = DAY2_CONFIGURATORS.get("ldap")


@verify_and_execute_configurator(config=LDAP_CONFIGURATOR, config_keys=["bind_password"], logger=LOGGER)
...
@verify_and_execute_configurator(config=LDAP_CONFIGURATOR, config_keys=["idp_name", "bind_password"], logger=LOGGER)
def create_oath(idp_name: str, bind_password: str):
    pass
...
def execute_ldap_configuration() -> Dict:
    LOGGER.info("Configuring LDAP")

    status_dict = {
        "Create LDAP secret": create_ldap_secret(
            bind_password=DAY2_CONFIG["configurators"]["ldap"].get("bind_password"),
        ),
    }
